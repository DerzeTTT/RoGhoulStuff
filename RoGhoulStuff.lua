...
ToggleTitle.Parent = AdditionalInfoOPTIONS
ToggleTitle.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
ToggleTitle.BackgroundTransparency = 1.000
ToggleTitle.Position = UDim2.new(0.0218749996, 0, 0, 0)
ToggleTitle.Size = UDim2.new(0.978124976, 0, 0.138248846, 0)
ToggleTitle.Font = Enum.Font.GothamSemibold
ToggleTitle.Text = "Friendly Fire: "
...
ToggleButton.Position = UDim2.new(0.735029936, 0, 0.0715822801, 0)
ToggleButton.Size = UDim2.new(0.239616618, 0, 0.833333313, 0)
ToggleButton.Font = Enum.Font.Code
ToggleButton.Text = "[OFF]"
ToggleButton.TextColor3 = Color3.fromRGB(250, 255, 234)
...
local Toggle = true
local FriendlyFireToggle = false
local InGameFriends = {}
local FullProfile
...
ToggleButton.MouseButton1Click:Connect(function()
	if not FriendlyFireToggle then
		ToggleButton.Text = "[ON]"
		FriendlyFireToggle = true
	else
		ToggleButton.Text = "[OFF]"
...
wait(1)
if FriendlyFireToggle then
for i,v in pairs(Players:GetPlayers()) do
if v:IsFriendsWith(Player.UserId) and not InGameFriends[v.Name] then				
            local string_1 = v.Name;
            local Target = game:GetService("ReplicatedStorage").Remotes.Ally.ToggleAlly;
            Target:InvokeServer(string_1);
table.insert(InGameFriends, #InGameFriends+1, v.Name)
print(v.Name.." has been allied")
		end
end
else
			for i,v in pairs(InGameFriends) do				
            local string_1 = v.Name;
            local Target = game:GetService("ReplicatedStorage").Remotes.Ally.ToggleAlly;
            Target:InvokeServer(string_1);
table.remove(InGameFriends, #InGameFriends-1)
print("Friends have been unallied")
		end
end
end)